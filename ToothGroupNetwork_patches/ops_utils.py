...
def get_clustering_labels(moved_points, labels):
    ...
    sub_points = moved_points[super_point_cond, :]
    if sub_points.shape[0] == 0:
        return np.zeros(0, dtype=int)

    clustering = DBSCAN(eps=0.03, min_samples=30).fit(sub_points, 3)
    ...
    if np.all(clustering_labels == -1):
        return np.zeros_like(clustering_labels)
    ...
    prb_cluster_num_ls = []
    if eg_values_first_axis.shape[0] >= 4:
        tail_mean = eg_values_first_axis[3:].mean()
        if np.isfinite(tail_mean) and tail_mean > 0:
            for i in range(min(3, eg_values_first_axis.shape[0])):
                if eg_values_first_axis[i] / tail_mean > 8:
    ...
    missing_mask = clustering_labels == -1
    if np.any(missing_mask):
        tree = KDTree(clustering_labeled_moved_points[valid_mask, :3], leaf_size=2)
        nn_neighbor_idxes = tree.query(clustering_labeled_moved_points[missing_mask, :3], k=10, return_distance=False)
        nn_neighbors_labels = clustering_labels[valid_mask][nn_neighbor_idxes]
        ...
        clustering_labels[missing_mask] = np.array(mod_labels)

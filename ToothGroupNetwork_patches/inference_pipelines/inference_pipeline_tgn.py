        ...
        if np.where(first_sem_label==1)[0].shape[0] + np.where(first_sem_label==9)[0].shape[0] > 20:
            ...
            for i in range(2,9):
                if np.where(first_sem_label==i)[0].shape[0]>20:
                    ...
                    break
        if centerpoints_of_11_12 is None:
            if ins_label_center_points.size > 0:
                centerpoints_of_11_12 = np.mean(ins_label_center_points, axis=0)
            elif np.isfinite(teeth_mean).all():
                centerpoints_of_11_12 = teeth_mean.reshape(3)
            else:
                centerpoints_of_11_12 = np.mean(first_xyz, axis=0)

            
        if ins_label_center_points.size > 0:
            ins_center_mean = np.mean(ins_label_center_points, axis=0)
        else:
            ins_center_mean = np.mean(first_xyz, axis=0)
        center_line = centerpoints_of_11_12 - ins_center_mean
